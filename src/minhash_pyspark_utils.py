...
def is_memory_safe(
    ...
    # Collect the sample to the driver node and measure the size of the sample
    ...
    print(f"Memory usage - {obj_name}: {est_size_gb:.5f}/{MAX_MEM_SIZE} GB [{est_size_mb:.5f} MB]")
    if est_size_gb < MAX_MEM_SIZE:
        return est_size_gb, True
    return est_size_gb, False
